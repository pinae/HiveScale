...
play, the account-claim flow, and the stats page). Next: wire e2e into CI and
**WP-13 (research export)**.

Partial in WP-11 (APIs/services + tests done; wider UI/endpoints still to come): the Daily Wave has a generator + share string but not yet the `/api/daily-wave/` play endpoints; content submission has the gated API + moderation queue but not yet an in-app submission form. The account-claim UI now ships (a "Save progress" panel plus magic-link confirmation on load), so WP-12's claim-preserves-score scenario is covered; the remaining deferred scenario — Daily-Wave share-to-clipboard — lands once those play endpoints exist.
